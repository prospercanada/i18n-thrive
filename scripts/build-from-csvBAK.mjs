...
if (!csvPath)
  throw new Error(
    "Usage: node scripts/build-from-csv.mjs <csvPath> [outDir] [namespace]"
  );
...
const body =
  /^key\s*,\s*en\s*,\s*fr\s*,\s*selector\s*,\s*type\s*,\s*attr$/.test(header)
    ? lines.slice(1)
    : lines;
...
const nsGuess =
  nsArg || filename.replace(/-translated\.csv$/i, "").replace(/\.csv$/i, ""); // profile-translated.csv -> profile
...
fs.writeFileSync(
  ...
  JSON.stringify(frSorted, null, 2) + "\n"
);
fs.writeFileSync(
